<template>
  <v-container>
    ...
    <v-col>
      ...
      <v-divider></v-divider>
      <v-carousel light height="600" hide-delimiters>
...
export default {
  ...
  async mounted() {
    ...
    console.log("state.authToken", this.$store.getters.getAuthToken);
    this.fetchUrl("eg", "sports")
      .then(response => response.json())
      .then(data => (this.articlesEgSports = data.articles));
    this.fetchUrl("eg", "business")
      .then(response => response.json())
      .then(data => (this.articlesEgBusiness = data.articles));
    this.fetchUrl("ae", "sports")
      .then(response => response.json())
      .then(data => (this.articlesAeSports = data.articles));
    this.fetchUrl("ae", "business")
      .then(response => response.json())
      .then(data => (this.articlesAeBusiness = data.articles));
  },
  methods: {
    goToFav(){
